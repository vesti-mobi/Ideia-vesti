...
/* Períodos de GMV (vêm do BigQuery, 1x/dia). O 3º está EM CURSO — hoje são
   poucos dias de 184 —, então crescimento se compara por MÉDIA MENSAL (ritmo
   contra ritmo). Comparar o total de 24 dias com o de 6 meses daria -90% para
   todo mundo e o ranking não serviria pra nada até dezembro. */
var BQ=(D.bq||{}), PER=(BQ.periodos||{});
var MESES_P1=6, MESES_P2=6;
function mesesP3(){ return Math.max(0.2,(BQ.p3_dias||1)/30.4375); }
function rotuloPer(p,fb){ var x=PER[p]; if(!x||!x[0]) return fb;
  var m=function(s){ var d=pdate(s); return d?pad2(d.getMonth()+1)+"/"+String(d.getFullYear()).slice(2):""; };
  return m(x[0])+"–"+m(x[1]); }
function porMes(v,meses){ return (v===null||v===undefined||meses<=0)?null:v/meses; }

function eff3row(r){
  var k=r.__key||("a3:"+r.cs_tab+":"+r.empresa), o=ov(k);
  var g1=toNum(pick(o.gmv_p1,r.gmv_p1)), g2=toNum(pick(o.gmv_p2,r.gmv_p2)), g3=toNum(pick(o.gmv_p3,r.gmv_p3));
  var m2=porMes(g2,MESES_P2), m3=porMes(g3,mesesP3());
  var cr = (o.cresc_rs!==undefined&&o.cresc_rs!==null&&o.cresc_rs!=="") ? toNum(o.cresc_rs)
         : ((m2!==null&&m3!==null) ? m3-m2 : null);
  var cp = (o.cresc_pct!==undefined&&o.cresc_pct!==null&&o.cresc_pct!=="") ? toNum(o.cresc_pct)
         : ((m2 && m3!==null) ? (m3-m2)/m2*100 : null);
  ...
  return { key:k, added:!!r.__key, src:r, hidden:!!o.hidden,
    ...
    plano:pick(o.plano,r.plano),
    gmv_p1:g1, gmv_p2:g2, gmv_p3:g3, cresc_rs:cr, cresc_pct:cp,
...
function renderA3(){
  ...
  var base =sortRows(rows.filter(function(r){return !r.added;}),s3,{
    ...
    gmv_p1:function(r){return r.gmv_p1;}, gmv_p2:function(r){return r.gmv_p2;}, gmv_p3:function(r){return r.gmv_p3;},
  ...
  var cols=[{label:"",cls:"strip",title:"clique na faixa para pintar a linha"},
    ...
    {label:"Plano",key:"plano",sort:1},
    {label:"GMV "+rotuloPer("p1","07/25–12/25"),key:"gmv_p1",sort:1,num:1,title:"GMV do 1º semestre anterior (fonte: BigQuery, atualiza 1x/dia)"},
    {label:"GMV "+rotuloPer("p2","01/26–06/26"),key:"gmv_p2",sort:1,num:1,title:"GMV do semestre anterior (fonte: BigQuery)"},
    {label:"GMV "+rotuloPer("p3","07/26–12/26"),key:"gmv_p3",sort:1,num:1,
      title:"Período EM CURSO: "+(BQ.p3_dias||0)+" dia(s) corridos. Por isso o crescimento compara média mensal, não o total."},
    {label:"Cresc. R$/mês",key:"cresc_rs",sort:1,num:1,title:"Diferença de R$ por mês entre o período atual e o anterior"},
    {label:"Cresc. %",key:"cresc_pct",sort:1,num:1,title:"Variação da média mensal (ritmo x ritmo), não do total"},
    {label:"Mensalidade"},
  ...
  added.concat(base).forEach(function(r){
    ...
    tr.appendChild(editCell(key,"gmv_p1",r.src.gmv_p1,{cls:"num mut",type:"num",ph:"0",fmt:money,onChange:re3}));
    tr.appendChild(editCell(key,"gmv_p2",r.src.gmv_p2,{cls:"num mut",type:"num",ph:"0",fmt:money,onChange:re3}));
    tr.appendChild(editCell(key,"gmv_p3",r.src.gmv_p3,{cls:"num",type:"num",ph:"0",fmt:money,onChange:re3}));
  ...
  tbl.appendChild(tb);
  if(!rows.length) tbl.innerHTML+='<tbody><tr><td colspan="14" class="empty">Nenhuma empresa com esses filtros.</td></tr></tbody>';
