...
LOG_FILE = "/home/diego/crons/sync_cities.log"
logging.basicConfig(
    filename=LOG_FILE,
    level=logging.INFO,
...
CSV_FILE = "/tmp/spooling_temp_cities.csv"

# ATENÇÃO: a tabela `cities` NÃO tem created_at/updated_at, então não há como
# fazer carga incremental por data. Como é uma tabela de referência e quase
# estática, fazemos recarga COMPLETA diária via WRITE_TRUNCATE.
QUERY = """
SELECT
    id, state_id, name
FROM cities
...
def run_sync():
    logging.info("--- Iniciando sincronização de cities (recarga completa diária) ---")
